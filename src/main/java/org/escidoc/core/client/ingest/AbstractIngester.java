...
public abstract class AbstractIngester implements Ingester {
	...
	ContentModelHandlerClientInterface cmc = new ContentModelHandlerClient(
		this.eSciDocInfrastructureBaseUrl);
	cmc.setHandle(this.userHandle);
	cmc.setTransport(TransportProtocol.REST);

	SearchRetrieveRequestType request = new SearchRetrieveRequestType();
	// request.setQuery("type = context");
	Collection<ContentModel> contextList = cmc
		.retrieveContentModelsAsList(request);
	...
	if (this.getInitialLifecycleStatus() != null
		&& !result.contains(this.getInitialLifecycleStatus())) {
	...
	if (this.containerContentModel == null
		|| this.containerContentModel.trim().length() == 0) {
	    throw new ConfigurationException(
		    "containerContentModel must be set.");
	}

	if (this.itemContentModel == null
		|| this.itemContentModel.trim().length() == 0) {
	    throw new ConfigurationException("itemContentModel must be set.");
	}

	if (this.contentCategory == null
		|| this.contentCategory.trim().length() == 0) {
	...
	if (this.ingestHandler == null) {
	    this.ingestHandler = new IngestHandlerClient(
		    this.geteSciDocInfrastructureBaseUrl());
	...
	if (this.containerHandler == null) {
	    this.containerHandler = new ContainerHandlerClient(
		    this.geteSciDocInfrastructureBaseUrl());
	...
	if (this.itemHandler == null) {
	    this.itemHandler = new ItemHandlerClient(
		    this.geteSciDocInfrastructureBaseUrl());
	...
	if (this.contentModelHandler == null) {
	    this.contentModelHandler = new ContentModelHandlerClient(
		    this.geteSciDocInfrastructureBaseUrl());
	...
	if (this.stagingHandler == null) {
	    this.stagingHandler = new StagingHandlerClient(
		    this.geteSciDocInfrastructureBaseUrl());
    ...
    protected final void seteSciDocInfrastructureBaseUrl(
	    final URL eSciDocInfrastructureBaseUrl)
	    throws ConfigurationException {
	if (this.eSciDocInfrastructureBaseUrl != null) {
	    throw new ConfigurationException(
